...
# (DO NOT TRY TO USE A MAKEFILE -- It's not worth the effort.)

require(knitr)
require(extrafont)
#pdfFonts('ComputerModern' = postscriptFonts('ComputerModern')$ComputerModern)
...
%Background noise can cause difficulty in identifying areas
%where munitions use occurred.
Additionally, mapping the level of contamination across the site after
sampling requires selecting the size of a search window in which the software
computes a moving average of spatial point density. This window size parameter
has no direct connection to the conceptual site model, so it is also of
interest to assess the importance of the window size choice.
...
% define detected and undetected
...
at \url{https://www.github.com/kflagg/vspuxo}.}
% Case studies~(Section~\ref{cases})
Finally, discussions of future investigations and comparisons with other
analysis methods contribute to the third goal~(Section~\ref{disc}).
...
%\emph{TODO: Summarize findings.
%Questions: What are some guidlines for choosing a good window size?
%How much does the anomaly density affect VSP's ability to detect target areas?
%How important is it to have good prior information about the anomaly density
%and the target area size?
%}
...
%\emph{TODO: Find a citation for UXO Estimator (IGD 14-01 has a link to a
%USACE web address that won't load for me -- private intranet?)}
...
%\emph{TODO: Describe history of VSP and why you would/would not use it.
%Goal: Convince Chris Barbour that a team would prefer to use VSP themselves
%rather than consult statisticians.}
...
The map~(left) was imported via a shapefile.% It was created in
%R~\parencite{baser} and exported by way of the maptools~\parencite{maptools}
%and rgdal~\parencite{rgdal} packages.
The room~(right) was created entirely within VSP.\nocite{rgdal,maptools}}
\label{maproom}
\end{figure}
...
\pagebreak %awkward place to break
...
proportion of anomalies. % and a distribution for locations of anomalies in
% the target area (Uniform or Bivariate Normal).
The TA is assumed to be circular or elliptical with a random orientation. The
user also specifies an interval of transect spacings over which VSP evaluates
the detection probability. VSP then plots the detection probability curve as
a function of transect spacing and the user clicks a point on the curve to
set the spacing for the sampling plan.~(Figure~\ref{onecurve}).

%The user can alternatively choose to view the detection probability as a
%function of the anomaly density in the target area instead of the transect
%spacing. This option can be used to assess the ability of a predetermined
%sampling plan to detect the target area.
...
%The window
%length controls how much area is used to compute the anomaly densities
%observed from the simulated sampling plan. The length defaults to 90\% of
%the target area minor axis.
...
Monte Carlo error.}
%These curves were generated by VSP and then plotted together in R, using the
%RDOMClient package~\parencite{dcom} to interface between the two software
%packages.
\label{curvsens}
\end{figure}
...
% Kriging smooths further? No, it gives a smooth surface that interpolates.
% Smoothing comes from the window size.

\begin{figure}[p!]
<<windows1,cache=TRUE>>=
#ldens <- read.geoeas('datasets/easy/exp2/rep2000_s0390.dat')
...
#xl <- c(1558000, 1560000, 1562000, 1564000)
#yl <- c(535000, 537000, 539000, 541000)
axis(1, at = axTicks(1),
...
#  plot(disc(0.45, c(centers[i, 'x'], centers[i, 'y'])),
#       border = '#00000040', add = TRUE)
#}
...
%The documentation does not describe the algorithm used to create the
%polygons, but it appears to connect the corners of cells that extrude from
%the cluster. It forces some regions to be convex by including lower-density
%cells, while other regions are allowed to be non-convex. It is also able to
%merge nearby regions into larger regions.

%GAMV computes the empirical semivariogram from the observed window
%densities, and then VSP fits parametric semivariogram models to the GAMV
%output. VSP considers spherical, exponential, Gaussian, and power
%semivariograms; the best fitting of these is selected automatically, but
%the documentation does not explain how the parameters are chosen and the
%different models are compared. Figure~\ref{geomap}~(left) shows an example
%where VSP has selected a Gaussian semivariogram. In this example, the
%parametric curve fits the points well. However, VSP occasionally chooses
%parameters that give poor fits, so the ``Map Anomaly Density'' tab includes
%a message advising the user that the plot should always be examined. The
%user can adjust the model and parameters if desired.
...
%Another popular version of Kriging is
%indicator Kriging, which predicts the probability that \(Z(\mathbf{s})\)
%is above or below a fixed threshold given whether each observed
%\(Z(\mathbf{s}_i)\) is below the threshold.
...
% Removed references to IK because I don't think VSP's IK is actually IK
% (It uses KT3D's OK function and I don't know what it really does)
...
modeling covariance functions and semivariograms. The techniques used by Visual Sample Plan originate in geostatistics and emphasize the semivariogram over
the covariance function, so the rest of this section centers around
...
%The issue of
%parametric semivariogram modeling is best summarized by stating
%that it is very complicated. Statistically justified estimators can be
%difficult to obtain and implement, so many simplifications are made out
%of necessity.
%For this study, Cressie's WLS approximation is used because
%it is simple to implement and is more robust than OLS when there are few
%pairs of observations at some lags.
...
%Operation of the software is very simple. The
%program reads a parameter file, then reads the data file, processes the
%data, and saves the results to a file. If no parameter file is provided,
%the routine outputs an example parameter file. The core GSLIB programs run
%on the command line, but a graphical front-end is available for Windows.

%The GSLIB routines used by Visual Sample Plan are GAMV, which computes
%empirical semivariograms, and KT3D, which does Kriging. The versions included
%with VSP are 32-bit and both report their version numbers as version 2.000.
%VSP also includes graphical interfaces for both programs, which bear the
%message ``Copyright 2007 Sandia Corporation.'' As of this writing, the
%latest 64-bit versions available from the GSLIB website are GAMV 2.905
%and KT3D 2.907.
...
%\emph{TODO: Describe assumptions (including prior information considered
%fixed or known for the purposes of the experiment), treatment levels, and
%response variables; clarify and organize the description of the
%implementation (it will not stay as one long paragraph!)}
...
%\begin{figure}[H]
<<harddiagram,eval=FALSE,out.width='3.25in',fig.width=3.25,cache=TRUE,depedson='meddiagram'>>=
par(mar = c(2, 4, 0, 0))
plot(sitewindow, show.all = FALSE)
plot(ranch, col = '#E0E0E0', lty = 0, add = TRUE)
plot(TAs.medh, col = '#D0D0D0', add = TRUE)
...
axis(2, at = axTicks(2),
     labels = prettyNum(axTicks(2)-min(axTicks(2)), big.mark = ','))
@
%\caption{Schematic of the hard site.}
%\label{harddiagram}
%\end{figure}
...
%Several Poisson processes can be combined to describe the anomalies at a UXO
%site. A single tank or artillery round will produce a small cluster of debris.
%Repeated firing causes the debris to accumulate into a larger cluster of
%anomalies. Firing at a single target will produce anomalies concentrated
%at the target location. Flying debris and shots that miss the target result
%in anomalies spread around the area, with decreasing density away from the
%target. Anomalies tend to be located in a circular or elliptical target
%area, so the distribution is well described by a Gaussian intensity function.

%Activities involving multiple targets located near each other will produce
%overlapping clusters. The cluster sizes and densities could vary between
%clusters depending on the munitions used and the activities performed.
%The amalgamation of munitions debris over years of site use could result
%in a high anomaly density in an irregularly-shaped area. On the other hand,
%areas where one type of munition is fired at several adjacent targets in
%roughly the same time period may have an approximate Gaussian intensity
%function as the clusters would blend together, forming one large target area.

%Training and testing activities involving multiple targets at distant
%locations around the site will act as a cluster process, where each target
%area is a cluster. Target areas vary in size and density depending on the
%activities performed at each location. Target area location is related to
%the geography of the site and the needs of the site users, and thus is an
%inhomogeneous process. However, if there are only a few target areas
%present or there is information about how the targets were selected, their
%locations may be considered deterministic instead of random. In practice
%the goal of spatial analysis is to find the target areas by locating the
%anomalies within them, but the process that generates their locations is
%not of interest.

%Background noise occurs at lower density than anomalies related to
%munitions use. In some cases the processes that generate these are known,
%such as vehicles driving along a road through the site. In many other cases,
%these processes are unknown and not of interest, so background noise is
%considered a homogeneous Poisson process for simplicity.
...
% Maybe put this in the conclusion?
...
%\emph{Ideas for quantities to summarize:}

%{\it
%\begin{itemize}
%\item TOI detection rate -- proportion of true TOI in identified TAs
%\item proportion of TA area detected
%\item TA false positive proportion -- proportion of identified TAs that
%contain no true TOI (or cannot be justified in other ways?)
%\item error distance -- shortest distance of each false negative to an
%identified region -- plot distribution for all replicates or individually
%\item detection rates for individual TAs
%\item spatial density of TOI at location of each false negative -- value
%of Kriged surface, or value of true intensity function -- can plot, also
%average
%\item proportion of anomalies in an identified TA that are/are not
%TOI -- probably not useful (we expect many non-TOI) but might be interesting
%\item other spatial summaries/statistics?
%\item length of transects
%\item cost/benefit analysis of sampling plan
%\end{itemize}
%}

%\emph{Ideas for presentation: Boxplots or caterpillar plots?
%ANOVA/Gelman ANOVA? (Is finite-pop or superpop sd more meaningful in this
%situation? Is variability sensitive to choice of levels?) Case studies for
%some individual sites? Discuss normality and residual plots?}
...
%\emph{SRS of 200 replicates of the easy site, sampling with 225 foot transect
%spacing and analyzing each with all five window sizes. Took about 14 hours
%to run (the small windows go very slowly). No minimum area is set, so all
%distinct regions are kept. The tiny regions are an important consideration
%because the window size is a smoothness parameter. Conclusion: set window
%diameter to 90\% of the minor axis length of the smallest TA of interest.}
...
#plot(NULL, xlim = c(0, 1), ylim = c(0, 42),
#     main = 'Detection Rate of TOI Items',
#     ylab = '', yaxt = 'n', xlab = 'Detection Rate')
#for(w in window.sizes){
#    plot(hist(results$detect[results$win==w], plot = FALSE,
#              breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (5-which(window.sizes==w))*0.75, add = TRUE)
#}
#boxplot(detect ~ win, data = results, horizontal = FALSE, ylim = c(0, 1),
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Detection Rate of TOI Items',
#        xlab = 'Detection Rate', ylab = 'Window Diameter in Feet')
...
#     main = 'Proportion of True TA Area Detected',
#     ylab = '', yaxt = 'n', xlab = 'Proportion Detected')
#for(w in window.sizes){
#    plot(hist(results$detectarea[results$win==w]/area(TAs.easy), plot = FALSE,
#              breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (5-which(window.sizes==w))*0.7, add = TRUE)
#}
#boxplot(I(detectarea/area(TAs.easy)) ~ win, data = results,
#        horizontal = TRUE, ylim = c(0, 1),
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Proportion of True TA Area Detected
#(TA True Positive Proportion)',
#        xlab = 'Proportion', ylab = 'Window Diameter in Feet')
...
#     ylab = '', yaxt = 'n', xlab = 'Detection Rate')
#for(w in window.sizes){
#    plot(hist(results$detect1[results$win==w], plot = FALSE,
#              breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (5-which(window.sizes==w))*0.75, add = TRUE)
#}
#boxplot(detect1 ~ win, data = results, horizontal = FALSE, ylim = c(0, 1),
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Detection Rate of TOI Items from TA1',
#        xlab = 'Detection Rate', ylab = 'Window Diameter in Feet')
...
#     ylab = '', yaxt = 'n', xlab = 'Detection Rate')
#for(w in window.sizes){
#    plot(hist(results$detect2[results$win==w], plot = FALSE,
#              breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (5-which(window.sizes==w))*0.75, add = TRUE)
#}
#boxplot(detect2 ~ win, data = results, horizontal = FALSE, ylim = c(0, 1),
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Detection Rate of TOI Items from TA2',
#        xlab = 'Detection Rate', ylab = 'Window Diameter in Feet')
...
%\begin{figure}[b!]
<<exp1results3,eval=FALSE,cache=TRUE,dependson='exp1results1'>>=
options(scipen = 5)
par(mfrow = c(1, 2), mar = c(5, 1, 2, 0), las = 1, bty = 'n', lend = 1)

stripchart(identcount ~ win, data = results, col = dcolsa6,
           method = 'stack', pch = '.', offset = 0.08, ylim = c(1, 15),
...
histby(I(identarea/43560) ~ win, data = results,
       breaks = seq(0, 540, 9), xlim = c(0, 540),
       main = 'Total Area of Delineated Regions',
       xlab = 'Area in Acres',
       border = dcols, col = colsa6,
       qcol = cbind(cols, dcols, cols), space1 = 0.1)
abline(v = area(TAs.easy)/43560, col = 'darkred')
#plot(NULL, xlim = c(0, 540), ylim = c(0, 0.05),
#     main = 'Total Area of Delineated Regions',
#     ylab = '', yaxt = 'n', xlab = 'Area in Acres')
#for(w in window.sizes){
#    plot(hist(results$identarea[results$win==w]/43560, plot = FALSE,
#              breaks = seq(0, 540, 6)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.0008, add = TRUE)
#}
#boxplot(I(identarea/43560) ~ win, data = results,
#        horizontal = FALSE, log = 'y',
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Total Area of Delineated Regions',
#        ylab = 'Area in Acres', xlab = 'Window Diameter in Feet')

#medareas <- lapply(window.sizes, function(w){
#    return(sapply(areas[,paste0('w', w)], median, na.rm = TRUE)/43560)
#  })
#names(medareas) <- window.sizes
#meanareas <- lapply(window.sizes, function(w){
#    return(sapply(areas[,paste0('w', w)], mean, na.rm = TRUE)/43560)
#  })
#names(meanareas) <- paste0('w', window.sizes)
...
#boxplot(I(identarea/(43560*identcount)) ~ win, data = results)
#boxplot(medareas, horizontal = TRUE, log = 'x', yaxt = 'n', xaxt = 'n',
#        border = dcols, col = cols, ylim = c(0.1, 1000),
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Area of Smallest Delineated Region',
#        xlab = 'Area in Acres', ylab = '')
#histby(I(minarea/43560) ~ win, data = min.long,
#       breaks = seq(0, 540, 6), #xlim = c(1, 120),
#       main = 'Area of Smallest Delineated Region',
#       xlab = 'Area in Acres',
#       border = dcols, col = colsa6, qcol = cols, space1 = 0.1,
#       lwd = c(2, 3, 4, 3, 2))

par(mar = c(1, 4, 2, 1), las = 0)
caterplot(I(minarea/43560) ~ win, data = min.long, prettyx = TRUE,
          vertical = TRUE, log = TRUE, #xlim = c(0.01, 500),
          main = 'Area of Smallest Delineated Region',
          xlab = 'Area in Acres',
          bg = colsa6, col = cbind(cols, dcols, cols), out.col = dcols)
abline(h = c(area(TA1), area(TA2))/43560, col = 'darkred')
#boxplot(I(minarea/43560) ~ win, data = min.long,
#          log = 'y',
#          main = 'Area of Smallest Delineated Region',
#          xlab = 'Area in Acres',
#          bg = colsa6, col = cols, out.col = dcols)
#abline(v = area(TA1)/(43560*2), col = 'darkred')

#histby(I(maxarea/43560) ~ win, data = max.long,
#       breaks = seq(0, 540, 6),
#       main = 'Area of Largest Delineated Region',
#       xlab = 'Area in Acres',
#       border = dcols, col = colsa6, qcol = cols, space1 = 0.1,
#       lwd = c(2, 3, 4, 3, 2))
#axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
caterplot(I(maxarea/43560) ~ win, data = max.long, prettyx = TRUE,
          vertical =TRUE, log = TRUE, #xlim = c(0.01, 500),
          main = 'Area of Largest Delineated Region',
          xlab = 'Area in Acres',
          bg = colsa6, col = cbind(cols, dcols, cols), out.col = dcols)
...
#plot(NULL, xlim = c(0, 200), ylim = c(0, 0.3),
#     main = 'Mean Area of Distinct Delineated Regions',
#     ylab = '', yaxt = 'n', xlab = 'Area in Acres')
#for(w in window.sizes){
#    plot(hist(log(meanareas[[paste0('w', w)]]/43560), plot = FALSE,
#              breaks = seq(0, 441, 6)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.0008, add = TRUE)
#}
#boxplot(meanareas, horizontal = TRUE, log = 'x', yaxt = 'n', xaxt = 'n',
#        border = dcols, col = cols, ylim = c(0.1, 1000),
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Mean Area of Delineated Regions',
#        xlab = 'Area in Acres', ylab = '')
#axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
#abline(v = area(TAs.easy)/(43560*2), col = 'darkred')

#boxplot(I(1-detectarea/identarea) ~ win, data = results,
#        horizontal = FALSE, ylim = c(0, 1),
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Proportion of Delineated Area that is Not TA
#(TA False Positive Proportion)',
#        xlab = 'Proportion', ylab = 'Window Diameter in Feet')
@
%<<exp1legend,cache=TRUE,dependson='exp1results1',fig.height=0.4,out.height='0.4in'>>=
%@
%\caption{Summaries of the area and number of possible target areas
%identified in the window size experiment. The red lines represent the true
%values (Top row: 2 TAs, total area 49.7 acres. Bottom row: TA1 area 17.3
%acres, TA2 area 32.4 acres).}
%\label{exp1res1}
%\end{figure}
...
histby(I(1-detectarea/identarea) ~ win, data = results,
       ...
       border = dcols, col = colsa6,
       qcol = cbind(cols, dcols, cols), space1 = 0.1)
#plot(NULL, xlim = c(0, 1), ylim = c(0, 25),
#     main = 'False Positive Proportion',
#     ylab = '', yaxt = 'n', xlab = 'Proportion')
#for(w in window.sizes){
#    plot(hist(1-results$detectarea[results$win==w]/results$identarea[results$win==w], plot = FALSE,
#              breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (5-which(window.sizes==w))*0.7, add = TRUE)
...
# Add the true areas and the minimum possible for each window size
#par(mfrow = c(2, 5))
#for(w in window.sizes){
#  plot(I(detectarea/area(TAs)) ~ identcount, data = results,
#       subset = (win==w), main = paste(w, 'Foot Window Diameter'),
#       xlab = 'Number of Regions Identified',
#       ylab = 'Proportion of True TA Area Detected')
#}
#for(w in window.sizes){
#  plot(I(1-detectarea/identarea) ~ identcount, data = results,
#       subset = (win==w), main = paste(w, 'Foot Window Diameter'),
...
#plot(NULL, xlim = c(0, 600), ylim = c(0, 0.015),
#     main = 'Error Distances',
#     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
#for(w in window.sizes){
#    plot(hist(unlist(ndist[,paste0('w', w)]), plot = FALSE,
#              breaks = seq(0, 4550, 6)), freq = FALSE,
#         border = dcols[which(window.sizes==w)],
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.00025, add = TRUE)
#}
...
for the 1,500 Foot Window', xlab = 'Distance in Feet',
            border = dcols, col = colsa6, quantiles = FALSE, space1 = 0.1)
#plot(NULL, xlim = c(0, 4550), ylim = c(0, 0.0055),
#     main = 'Error Distances on a Scale Suitable
#for the 1,500 Foot Window',
#     ylab = '', yaxt = 'n', xaxt = 'n',
#     xlab = 'Distance in Feet')
#for(w in window.sizes){
#    plot(hist(unlist(ndist[,paste0('w', w)]), plot = FALSE,
#         breaks = seq(0, 4550, 50)), freq = FALSE,
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.00009, add = TRUE)
#}
#axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
...
histby.list(ndist['r84',], breaks = seq(0, 4536, 9), xlim = c(0, 600),
            ...
            xlab = 'Distance in Feet',
            border = dcols, col = colsa6, quantiles = FALSE, space1 = 0.1)
#plot(NULL, xlim = c(0, 600), ylim = c(0, 0.018),
#     main = 'Error Distances for Realization 84',
#     ylab = '', yaxt = 'n',
#     xlab = 'Distance in Feet')
#for(w in window.sizes){
#    plot(hist(ndist[['r84', paste0('w', w)]], plot = FALSE,
#         breaks = seq(0, 4550, 12)), freq = FALSE,
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.0003, add = TRUE)
#}
...
histby.list(ndist['r1984',], breaks = seq(0, 4536, 72),
            ...
            xlab = 'Distance in Feet',
            border = dcols, col = colsa6, quantiles = FALSE, space1 = 0.1)
#plot(NULL, xlim = c(0, 4550), ylim = c(0, 0.011),
#     main = 'Error Distances for Realization 1,984',
#     ylab = '', yaxt = 'n', xaxt = 'n',
#     xlab = 'Distance in Feet')
#for(w in window.sizes){
#    plot(hist(ndist[['r1984', paste0('w', w)]], plot = FALSE,
#         breaks = seq(0, 4550, 50)), freq = FALSE,
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.0002, add = TRUE)
#}
#axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
...
histby.list(ndist['r1021',], breaks = seq(0, 4536, 9), xlim = c(0, 600),
            ...
            xlab = 'Distance in Feet',
            border = dcols, col = colsa6, quantiles = FALSE, space1 = 0.1)
#plot(NULL, xlim = c(0, 600), ylim = c(0, 0.038),
#     main = 'Error Distances for Realization 1,021',
#     ylab = '', yaxt = 'n',
#     xlab = 'Distance in Feet')
#for(w in window.sizes){
#    plot(hist(ndist[['r1021', paste0('w', w)]], plot = FALSE,
#         breaks = seq(0, 4550, 12)), freq = FALSE,
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.0007, add = TRUE)
#}
...
histby.list(ndist['r2955',], breaks = seq(0, 4536, 9), xlim = c(0, 600),
            ...
            xlab = 'Distance in Feet',
            border = dcols, col = colsa6, quantiles = FALSE, space1 = 0.1)
#plot(NULL, xlim = c(0, 600), ylim = c(0, 0.016),
#     main = 'Error Distances for Realization 2,955',
#     ylab = '', yaxt = 'n',
#     xlab = 'Distance in Feet')
#for(w in window.sizes){
#    plot(hist(ndist[['r2955', paste0('w', w)]], plot = FALSE,
#         breaks = seq(0, 4550, 12)), freq = FALSE,
#         col = colsa6[which(window.sizes==w)],
#         voffset = (which(window.sizes==w)-1)*0.0003, add = TRUE)
#}
...
#par(mar = c(5, 5, 2, 1))
#flatareas <- lapply(window.sizes, function(w){
#    return((unlist(areas[,paste0('w', w)]))/43560)
#  })
#names(flatareas) <- window.sizes
#boxplot((flatareas), horizontal = FALSE, log = 'y',
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Area of Individual Delineated Regions',
#        xlab = 'Area in Acres', ylab = 'Window Diameter in Feet')
#abline(v = c(area(TA1), area(TA2))/43560, col = 'darkred')
...
#     ylab = '', yaxt = 'n', xlab = 'Area in Acres')
#for(w in window.sizes){
#    hist(unlist(areas[,paste0('areas', w)])/43560, freq = FALSE,
#         breaks = c(0:100, 600),
#         col = colsa[which(window.sizes==w)], add = TRUE)
#}
@
...
%\begin{figure}[p!]
<<exp1areas1,eval=FALSE,cache=TRUE,dependson='exp1results1'>>=
options(scipen = 5)
par(mfrow = c(1, 2), mar = c(5, 2, 2, 0), las = 1, bty = 'n')

histby.array(acres, breaks = seq(0, 500, 10),
             main = 'Areas of Individual Regions',
             xlab = 'Area in Acres',
             border = dcols, col = colsa6)

histby.array(acres, breaks = 100,
             main = 'Areas of Individual Regions',
             xlab = 'Area in Acres',
             border = dcols, col = colsa6)

histby.list(acres['r84',], breaks = 100,
           main = 'Areas of Individual Regions for Realization 84',
           xlab = 'Area in Acres',
           border = dcols, col = colsa6)

histby.list(acres['r1984',], breaks = 100,
           main = 'Areas of Individual Regions for Realization 19,84',
           xlab = 'Area in Acres',
           border = dcols, col = colsa6)

histby.list(acres['r1021',], breaks = 100,
           main = 'Areas of Individual Regions for Realization 1,021',
           xlab = 'Area in Acres',
           border = dcols, col = colsa6)

histby.list(acres['r2955',], breaks = 100,
           main = 'Areas of Individual Regions for Realization 2,955',
           xlab = 'Area in Acres',
           border = dcols, col = colsa6)
@
%<<exp1legend,fig.height=0.4,out.height='0.4in'>>=
%@
%\caption{Distributions of the areas of individual delineated regions for all
%realizations combined and for several individual realizations.}
%\label{exp1areas}
%\end{figure}

<<moreideas,eval=FALSE,cache=FALSE,dependson='exp1results1'>>=
#par(mfrow = c(1, 2), mar = c(0, 0, 2, 0))
#plot(idpoints[marks(idpoints)>0], use.marks = FALSE,
#     pch = 4, cex = 0.5, cols = '#00000040', main = 'Detected TOI Items')
#plot(missedpoints[marks(missedpoints)>0], use.marks = FALSE,
#     pch = 4, cex = 0.5, cols = '#00000040', main = 'Missed TOI Items')

# Other ideas: Proportion of site delineated, distribution of area of
# distinct regions, some residual plots.
@
...
background density. In Section~\ref{myplan}, I used each combination of these
...
large size or the size of A -- %, use transect spacings% of 780 feet, 785 feet, and 1,145 feet,
%respectively; these spacings are
%large enough to miss
%most or all of the 800 foot by 800 foot square field, so they
occasionally
result in very little or none of the ranch area being delineated. The ranch
is a true high density region similar in size to a target area, so it
should be expected to appear in the predicted density surface. % At a
%real site, it would not be appropriate to use the same sampling plan at the
%ranch as at the rest of the site.
The ranch would need to be sampled
...
%\subsection{Easy Site Results}
%\label{results2e}

%When assuming 100 anomalies per acre, the distribution of error distances
%is similar for all target area sizes~(Figures~\ref{exp2eres3} and
%\ref{exp2eres4}). As the anomaly density increases, the sampling plans
%based on TA2 and the too-large TA yield distributions with longer right
%tails. These analyses provide poor resolution for delineating the target
%areas, and for several realizations they completely failed to detect some
%target areas.

%For the easy site, it is more important to have accurate prior information
%about the target area sizes than the target area density.

%\begin{figure}[p!]
<<exp2eresults2,eval=FALSE,cache=TRUE,dependson='exp2eresults1',out.height='3.75in',fig.height=3.75>>=
...
histby(detect ~ Target + fg, data = results2e,
       ...
       border = dcolse, col = colsa6e,
       qcol = cbind(colse, dcolse, dcolse), space1 = 0.2)
#plot(NULL, xlim = c(0, 1), ylim = c(0, 95), yaxt = 'n',
#     main = 'Detection Rate of TOI Items',
#     ylab = 'Prior Anomaly Density Above Background', xlab = 'Detection Rate')
#for(f in fg.prior){
#  for(t in targets){
#    plot(hist(results2e$detect[results2e$Target==t & results2e$fg==f],
#              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(targets==t)],
#         col = colsa6[which(targets==t)],
#         voffset = (4-which(targets==t))+(which(fg.prior==f)-1)*40,
#         add = TRUE)
#  }
#}
#axis(2, at = c(1.5, 41.5, 81.5), tick = FALSE,
#     labels = paste(fg.prior, 'per Acre'))
...
#     main = 'Proportion of True TA Area Detected',
#     ylab = 'Prior Anomaly Density Above Background',
#     yaxt = 'n', xlab = 'Proportion Detected')
#for(f in fg.prior){
#  for(t in targets){
#    plot(hist(results2e$detectarea[results2e$Target==t & results2e$fg==f]/area(TAs.easy),
#              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(targets==t)],
#         col = colsa6[which(targets==t)],
#         voffset = (4-which(targets==t))*0.75+(which(fg.prior==f)-1)*22,
#         add = TRUE)
#    }
#}
#axis(2, at = c(1.125, 23.125, 45.125), tick = FALSE,
#     labels = paste(fg.prior, 'per Acre'))
...
#for(f in fg.prior){
#  for(t in targets){
#    plot(hist(results2e$detect1[results2e$Target==t & results2e$fg==f],
#              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(targets==t)],
#         col = colsa6[which(targets==t)],
#         voffset = (4-which(targets==t))+(which(fg.prior==f)-1)*40,
...
#for(f in fg.prior){
#  for(t in targets){
#    plot(hist(results2e$detect2[results2e$Target==t & results2e$fg==f],
#              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
#         border = dcols[which(targets==t)],
#         col = colsa6[which(targets==t)],
#         voffset = (4-which(targets==t))+(which(fg.prior==f)-1)*40,
...
%<<exp2elegendnotrue,fig.height=0.35,out.height='0.35in'>>=
%@
%\caption{TOI detection rates and numbers of distinct regions delineated
%for the easy site.}
%\label{exp2edetect}
%\end{figure}

%\begin{figure}[p!]
<<exp2eresults3,eval=FALSE,cache=TRUE,dependson='exp2eresults1',out.height='3.75in',fig.height=3.75>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

stripchart(identcount ~ Target + fg, data = results2e, col = colse,
           method = 'stack', pch = '.', offset = 0.05, ylim = c(1, 13),
           main = 'Number of Distinct Regions Identified',
           xlab = 'Number of Regions',
           ylab = 'Prior Anomaly Density above Background', yaxt = 'n')
axis(2, at = c(2.5, 6.5, 10.5), tick = FALSE,
     labels = paste(fg.priore, 'per Acre'))
...
histby(I(identarea/43560) ~ Target + fg, data = results2e,
       ...
       qcol = cbind(colse, dcolse, dcolse), space1 = 0.2)
abline(v = area(TAs.easy)/43560, col = 'darkred')
#plot(NULL, xlim = c(0, 540), ylim = c(0, 0.1),
#     main = 'Total Area of Delineated Regions',
#     ylab = 'Prior Anomaly Density Above Background',
#     yaxt = 'n', xlab = 'Area in Acres')
#for(f in fg.prior){
#  for(t in targets){
#    plot(hist(results2e$identarea[results2e$Target==t & results2e$fg == f]/43560,
#              plot = FALSE, breaks = seq(0, 540, 6)), freq = FALSE,
#         border = dcols[which(targets==t)],
#         col = colsa6[which(targets==t)],
#         voffset = (4-which(targets==t))*0.00125+(which(fg.prior==f)-1)*0.035,
#         add = TRUE)
#    }
#}
#axis(2, at = c(0.001875, 0.036875, 0.071875), tick = FALSE,
#     labels = paste(fg.prior, 'per Acre'))

#medareas <- sapply(fg.prior, function(f){
#  return(sapply(targets, function(t){
#      return(sapply(areas2e[,paste0('Target', t),paste0('fg', f)],
#                    median, na.rm = TRUE)/43560)
#    }))
#  }, simplify = 'array')
#dimnames(medareas)[[3]] <- fg.prior
medareas <- sapply(fg.priore, function(f){
  return(lapply(targetse, function(t){
      return(sapply(areas2e[,paste0('Target', t), paste0('fg', f)],
                      median, na.rm = TRUE)/43560)
    }))
  })
rownames(medareas) <- targetse
colnames(medareas) <- fg.priore
names(medareas) <- sapply(targetse, function(t){
    return(sapply(fg.priore, function(f){
        return(paste(t, f, sep = '.'))
      }))
  })
#boxplot(array(medareas), horizontal = TRUE, log = 'x', yaxt = 'n',
#        border = dcols, col = cols,
#        outpch = 21, outcol = colsa6, outbg = colsa4,
#        main = 'Median Area of Delineated Regions',
#        xlab = 'Area in Acres',
#        ylab = 'Prior Anomaly Density Above Background')
#axis(2, at = c(2.5, 6.5, 10.5), tick = FALSE,
#     labels = paste(fg.prior, 'per Acre'))
...
meanarease <- sapply(fg.priore, function(f){
  return(lapply(targetse, function(t){
      return(sapply(areas2e[,paste0('Target', t), paste0('fg', f)],
                      mean, na.rm = TRUE)/43560)
    }))
  })
rownames(meanarease) <- targetse
colnames(meanarease) <- fg.priore
names(meanareas) <- sapply(targetse, function(t){
    return(sapply(fg.priore, function(f){
        return(paste(t, f, sep = '.'))
      }))
  })
boxplot(array(meanarease), horizontal = TRUE, log = 'x', yaxt = 'n',
        border = dcolse, col = colse,
        outpch = 21, outcol = colsa6e, outbg = colsa4e,
        main = 'Mean Area of Delineated Regions',
        xlab = 'Area in Acres',
        ylab = 'Prior Anomaly Density Above Background')
axis(2, at = c(2.5, 6.5, 10.5), tick = FALSE,
...
#factorcombos <- expand.grid(fg.prior, targets)
#minareas <- sapply(seq_along(nrow(factorcombos)), function(i){
#    return(sapply(areas2e[,paste0('Target', factorcombos[i,2]),
#                          paste0('fg', factorcombos[i,1])],
#                  min, na.rm = TRUE))
#  })
#colnames(minareas) <- paste0('win', window.sizes)
#min.long <- reshape(as.data.frame(minareas), direction = 'long',
...
minarease <- sapply(fg.priore, function(f){
  return(lapply(targetse, function(t){
      return(sapply(areas2e[,paste0('Target', t), paste0('fg', f)],
                      min, na.rm = TRUE)/43560)
    }))
  })
rownames(minarease) <- targetse
colnames(minarease) <- fg.priore
names(minarease) <- sapply(targetse, function(t){
    return(sapply(fg.priore, function(f){
        return(paste(t, f, sep = '.'))
      }))
  })
...
abline(h = c(area(TA1), area(TA2))/43560, col = 'darkred')
@
%<<exp2elegend,fig.height=0.3,out.height='0.3in'>>=
%@
%\caption{Summaries of the easy site results. The red vertical lines
%represent the true values (2 TAs, total area 49.7 acres, median and mean
%area 24.9 acres).}
%\label{exp2eres1}
%\end{figure}

%\begin{figure}[p!]
<<exp2eresults4,eval=FALSE,cache=TRUE,dependson='exp2eresults1',out.height='3.75in',fig.height=3.75>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

plot(NULL, xlim = c(0, 1), ylim = c(0, 55),
     main = 'Proportion of True TA Area Detected',
     ylab = 'Prior Anomaly Density Above Background',
     yaxt = 'n', xlab = 'Proportion')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2e$detectarea[results2e$Target==t & results2e$fg == f]/area(TAs.easy),
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.7+(which(fg.prior==f)-1)*19,
         add = TRUE)
    }
}
axis(2, at = c(1.05, 20.05, 39.05), tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
# Plotting against the number of regions gives an unhelpful mess of a plot
#plot(I(detectarea/area(TAs.easy)) ~ identcount, data = results2e,
#     col = colsa6[Target], bg = colsa4[Target],
#     pch = c(21, 23, 24)[factor(fg)], ylim = c(0, 1),
...
plot(NULL, xlim = c(0, 1), ylim = c(0, 55),
     main = 'False Positive Proportion',
     ylab = 'Prior Anomaly Density Above Background',
     yaxt = 'n', xlab = 'False Positive Proportion')
for(f in fg.prior){
  for(t in targets){
    plot(hist(1-results2e$detectarea[results2e$Target==t & results2e$fg == f]/results2e$identarea[results2e$Target==t & results2e$fg == f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.7+(which(fg.prior==f)-1)*19,
...
axis(2, at = c(1.05, 20.05, 39.05), tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
#plot(I(1-detectarea/identarea) ~ identcount, data = results2e,
#     col = colsa6[Target], bg = colsa4[Target],
#     pch = c(21, 23, 24)[factor(fg)], ylim = c(0, 1),
#     main = 'False Positive Rate vs
#Number of Distinct Regions Identified',
#     xlab = 'Number of Regions',
#     ylab = 'False Positive Rate')
#abline(v = 2, col = 'darkred')
@
%<<exp2elegendnotrue,eval=FALSE,fig.height=0.35,out.height='0.35in'>>=
%@
%\caption{TA detection rate and TA false positive rate for the easy site.}
%\label{exp2eres2}
%\end{figure}

%\begin{figure}[p!]
<<exp2eresults5,eval=FALSE,cache=TRUE,dependson='exp2eresults1',out.height='3.75in',fig.height=3.75>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

histby(ndist2e, breaks = seq(0, 6000, 8), xlim = c(0, 800),
       main = 'Distance from Missed TOI to Nearest
Delineated Region (Error Distance)',
       ylab = 'Prior Anomaly Density Above Background',
       xlab = 'Distance in Feet',
       labels = paste(fg.priore, 'per Acre'),
       border = dcolse, col = colsa6e)
#plot(NULL, xlim = c(0, 800), ylim = c(0, 0.045),
#     main = 'Distance from Missed TOI to Nearest
#Delineated Region (Error Distance)',
#     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
#for(f in fg.prior){
#  for(t in targets){
#    plot(hist(unlist(ndist2e[,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
#              breaks = seq(0, 6000, 8)), freq = FALSE,
#         border = dcols[which(targets==t)],
#         col = colsa6[which(targets==t)],
#         voffset = (which(targets==t)-1)*0.00075+(which(fg.prior==f)-1)*0.015,
#         add = TRUE)
#  }
#}
#axis(2, at = c(0.001125, 0.016125, 0.031125), tick = FALSE,
#     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 5106), ylim = c(0, 0.00825),
     main = 'Histograms of Error Distances
...
for(f in fg.priore){
  for(t in targetse){
    plot(hist(unlist(ndist2e[,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 6000, 50)), freq = FALSE,
         border = dcolse[which(targetse==t)],
         col = colsa6e[which(targetse==t)],
         voffset = (which(targetse==t)-1)*0.00012+(which(fg.priore==f)-1)*0.0027,
         add = TRUE)
  }
}
axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
axis(2, at = c(0.00018, 0.00288, 0.00558), tick = FALSE,
     labels = paste(fg.priore, 'per Acre'))
# repl 1491 delineated 0 regions with tLarge fg200
#table(results2e[which(results2e$detect1==0|results2e$detect2==0),c('Target','fg')])
@
%<<exp2elegendnotrue,fig.height=0.35,out.height='0.35in'>>=
%@
%\caption{Distributions of error distance for the easy site. The clusters of
%large distances in the plot on the right come from 22 realizations where an
%analysis failed to detect TA1, 3 realizations where an analysis failed to
%detect TA2, and one realization where one analysis failed to delineate any
%part of either target area.}
%\label{exp2eres3}
%\end{figure}

%\begin{figure}[p!]
<<exp2eresults6,eval=FALSE,cache=TRUE,dependson='exp2eresults1',out.height='3.75in',fig.height=3.75>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

plot(NULL, xlim = c(0, 800), ylim = c(0, 0.059),
     main = 'Error Distances for Realization 1,209',
     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
for(f in fg.priore){
  for(t in targetse){
    plot(hist(unlist(ndist2e['r1209' ,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 6000, 8)), freq = FALSE,
         border = dcolse[which(targetse==t)],
         col = colsa6e[which(targetse==t)],
         voffset = (which(targetse==t)-1)*0.00075+(which(fg.priore==f)-1)*0.019,
         add = TRUE)
  }
}
axis(2, at = 1.5*0.00075+(0:2)*0.019, tick = FALSE,
     labels = paste(fg.priore, 'per Acre'))

plot(NULL, xlim = c(0, 5106), ylim = c(0, 0.025),
     main = 'Error Distances for Realization 28',
     ylab = '', yaxt = 'n', xaxt = 'n',
     xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2e['r28', paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 6000, 50)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00036+(which(fg.prior==f)-1)*0.0081,
         add = TRUE)
  }
}
axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
axis(2, at = 1.5*0.00036+(0:2)*0.0081, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 800), ylim = c(0, 0.066),
     main = 'Error Distances for Realization 1,363',
     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2e['r1363' ,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 6000, 8)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00075+(which(fg.prior==f)-1)*0.022,
         add = TRUE)
  }
}
axis(2, at = 1.5*0.00075+(0:2)*0.022, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 5106), ylim = c(0, 0.025),
     main = 'Error Distances for Realization 2,541',
     ylab = '', yaxt = 'n', xaxt = 'n',
     xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2e['r2541', paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 6000, 50)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00036+(which(fg.prior==f)-1)*0.0081,
...
%<<exp2elegendnotrue,fig.height=0.35,out.height='0.35in'>>=
%@
%\caption{Distributions of error distance for several realizations of the
%easy site. For realization 28, the sampling plan based on the too-large target
%area and 200 anomalies per acre above background results in one possible
%target area being identified. This 20.93 acre region includes 69.4\% of TA1
%but does not include any part of TA2. For realization 2541, the sampling
%plan based on the too-large target area and 400 anomalies per acre above
%background results in the identification of 3 possible target areas
%totaling 39.77 acres, but none of these intersect the either of the true
%target areas.}
%\label{exp2eres4}
%\end{figure}

%\begin{figure}[t!]
<<exp2eresults7,eval=FALSE,cache=TRUE,dependson='exp2eresults1',out.height='3.75in',fig.height=3.75>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

histby(I(1-results2e$detectarea/results2e$identarea) ~ Target + fg,
       data = results2e, breaks = seq(0, 1, 0.02), xlim = c(0, 1),
       main = 'False Positive Proportion',
       ylab = 'Prior Anomaly Density Above Background',
       xlab = 'False Positive Proportion',
       labels = paste(fg.prior, 'per Acre'),
       border = dcols, col = colsa6,
       qcol = cbind(cols, dcols, dcols), space1 = 0.2)
#plot(NULL, xlim = c(0, 1), ylim = c(0, 55),
#     main = 'False Positive Proportion',
#     ylab = 'Prior Anomaly Density Above Background',
#     yaxt = 'n', xlab = 'False Positive Proportion')
...
%<<exp2elegendcirc,fig.height=0.5,out.height='0.5in'>>=
%@
%\caption{Plots illustrating the sampling effort and efficiency for the
%easy site.}
%\label{exp2eres5}
%\end{figure}

%\subsection{Medium Site Results}
%\label{results2m}

%As the estimated target area size increases, the median and mean areas
%of the individual regions increase and their distributions shift from
%being right-skewed to left-skewed~(Figure~\ref{exp2mres1}, bottom). When
%the sampling plan is based on the too-small target area, the median and mean
%become less variable as the assumed anomaly density is increased, but
%otherwise the density does not affect the median or mean area. Sampling
%plans based on T1 generally identify potential target areas of the correct
%size.

%Error distances tend to be small, but the distributions have longer
%tails as the prior target area size increases~(Figures~\ref{exp2mres3} and
%\ref{exp2mres4}). This is because the sampling plans based on A and the
%too-large TA completely miss T1 and T2 for some realizations. There are no
%clear differences in error distance distribution among the prior anomaly
%densities.

%\begin{figure}[b!]
<<exp2mresults2,eval=FALSE,cache=TRUE,dependson='exp2mresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

plot(NULL, xlim = c(0, 1), ylim = c(0, 110), yaxt = 'n',
     main = 'Detection Rate of TOI Items',
     ylab = 'Prior Anomaly Density Above Background', xlab = 'Detection Rate')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2m$detect[results2m$Target==t & results2m$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.9+(which(fg.prior==f)-1)*45,
         add = TRUE)
  }
}
axis(2, at = 1.5*0.9+(0:2)*45, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 1), ylim = c(0, 110),
     main = 'Proportion of True TA Area Detected',
     ylab = 'Prior Anomaly Density Above Background',
     yaxt = 'n', xlab = 'Proportion Detected')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2m$detectarea[results2m$Target==t & results2m$fg==f]/area(union.owin(TAs.medh, TA2.medh)),
              plot = FALSE, breaks = seq(0, 1.02, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (4-which(targets==t))+(which(fg.prior==f)-1)*40,
         add = TRUE)
...
axis(2, at = 1.5+(0:2)*40, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
@
%<<exp2mlegend,fig.height=0.35,out.height='0.35in'>>=
%@
%\caption{Overall TOI detection rates and numbers of distinct regions
%delineated for the medium site.}
%\label{exp2mdetect}
%\end{figure}

%lots of interactions involving small & fg

%\begin{figure}[b!]
<<exp2mresults2b,eval=FALSE,cache=TRUE,dependson='exp2mresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

plot(NULL, xlim = c(0, 1), ylim = c(0, 125), yaxt = 'n',
     main = 'Detection Rate of TOI Items from T1',
     ylab = 'Prior Anomaly Density Above Background', xlab = 'Detection Rate')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2m$detect3[results2m$Target==t & results2m$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)+(which(fg.prior==f)-1)*50,
         add = TRUE)
  }
}
axis(2, at = 1.5+(0:2)*50, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 1), ylim = c(0, 150), yaxt = 'n',
     main = 'Detection Rate of TOI Items from A',
     ylab = 'Prior Anomaly Density Above Background', xlab = 'Detection Rate')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2m$detect5[results2m$Target==t & results2m$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (4-which(targets==t))*1.2+(which(fg.prior==f)-1)*60,
         add = TRUE)
  }
}
axis(2, at = 1.5*1.2+(0:2)*60, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 1), ylim = c(0, 150), yaxt = 'n',
     main = 'Detection Rate of TOI Items from T2',
     ylab = 'Prior Anomaly Density Above Background', xlab = 'Detection Rate')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2m$detect4[results2m$Target==t & results2m$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (4-which(targets==t))*1.2+(which(fg.prior==f)-1)*60,
...
plot(NULL, xlim = c(0, 1), ylim = c(0, 25), yaxt = 'n',
     main = 'Proportion of the Road Area Delineated',
     ylab = 'Prior Anomaly Density Above Background', xlab = 'Proportion')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2m$detectarea1[results2m$Target==t & results2m$fg==f]/area(road),
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.2+(which(fg.prior==f)-1)*10,
         add = TRUE)
  }
}
axis(2, at = 1.5*0.2+(0:2)*10, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
@
%<<exp2mlegend,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Detection rates by region for the medium site.}
%\label{exp2mdetectreg}
%\end{figure}

%\begin{figure}[p!]
<<exp2mresults3,eval=FALSE,cache=TRUE,dependson='exp2mresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

stripchart(identcount ~ Target + fg, data = results2m, col = cols,
           method = 'stack', pch = '.', offset = 0.03, ylim = c(1, 13),
           main = 'Number of Distinct Regions Identified',
           xlab = 'Number of Regions',
           ylab = 'Prior Anomaly Density above Background', yaxt = 'n')
axis(2, at = c(2.5, 6.5, 10.5), tick = FALSE,
...
plot(NULL, xlim = c(0, 700), ylim = c(0, 0.06),
     main = 'Total Area of Delineated Regions',
     ylab = 'Prior Anomaly Density Above Background',
     yaxt = 'n', xlab = 'Area in Acres')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2m$identarea[results2m$Target==t & results2m$fg == f]/43560,
              plot = FALSE, breaks = seq(0, 704, 8)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.001+(which(fg.prior==f)-1)*0.02,
         add = TRUE)
    }
}
axis(2, at = 1.5*0.001+(0:2)*0.02, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
abline(v = area(union.owin(TAs.medh, TA2.medh))/43560, col = 'darkred')

medareas <- sapply(fg.prior, function(f){
  return(lapply(targets, function(t){
      return(sapply(areas2m[,paste0('Target', t), paste0('fg', f)],
                    function(x){
                      return(median(ifelse(is.null(x), NA, x), na.rm = TRUE))
                    })/43560)
    }))
  })
rownames(medareas) <- targets
colnames(medareas) <- fg.prior
names(medareas) <- sapply(targets, function(t){
    return(sapply(fg.prior, function(f){
        return(paste(t, f, sep = '.'))
      }))
  })
boxplot(array(medareas), horizontal = TRUE, log = 'x', yaxt = 'n',
...
axis(2, at = c(2.5, 6.5, 10.5), tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
abline(h = c(4.5, 8.5), col = '#40404040')
abline(v = area(TA2.medh)/43560, col = 'darkred')

meanareas <- sapply(fg.prior, function(f){
  return(lapply(targets, function(t){
      return(sapply(areas2m[,paste0('Target', t), paste0('fg', f)],
                    function(x){
                      return(mean(ifelse(is.null(x), NA, x), na.rm = TRUE))
                    })/43560)
    }))
  })
rownames(meanareas) <- targets
colnames(meanareas) <- fg.prior
names(meanareas) <- sapply(targets, function(t){
    return(sapply(fg.prior, function(f){
        return(paste(t, f, sep = '.'))
      }))
  })
boxplot(array(meanareas), horizontal = TRUE, log = 'x', yaxt = 'n',
        border = dcols, col = cols,
        outpch = 21, outcol = colsa6, outbg = colsa4,
        main = 'Mean Area of Delineated Regions',
        xlab = 'Area in Acres',
...
axis(2, at = c(2.5, 6.5, 10.5), tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
abline(h = c(4.5, 8.5), col = '#40404040')
abline(v = area(union.owin(TAs.medh, TA2.medh))/(43560*3), col = 'darkred')
@
%<<exp2mlegend,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Summaries of the medium site results. The red vertical lines
%represent the true values (3 TAs, total area 62.9 acres, median area
%11.5 acres, mean area 21.0 acres).}
%\label{exp2mres1}
%\end{figure}

%\begin{figure}[H]
<<exp2mresults4,eval=FALSE,cache=TRUE,dependson='exp2mresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')
...
plot(NULL, xlim = c(0, 1), ylim = c(0, 65),
     main = 'False Positive Rate',
     ylab = 'Prior Anomaly Density Above Background',
     yaxt = 'n', xlab = 'False Positive Proportion')
for(f in fg.prior){
  for(t in targets){
    plot(hist(1-results2m$detectarea[results2m$Target==t & results2m$fg == f]/results2m$identarea[results2m$Target==t & results2m$fg == f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.7+(which(fg.prior==f)-1)*21,
         add = TRUE)
    }
}
axis(2, at = 1.5*0.7+(0:2)*21, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
@
%<<exp2mlegendnotrue,eval=FALSE,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{TA detection rate and TA false positive rate for the medium site.}
%\label{exp2mres2}
%\end{figure}

%\begin{figure}[p!]
<<exp2mresults5,eval=FALSE,cache=TRUE,dependson='exp2mresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

plot(NULL, xlim = c(0, 1500), ylim = c(0, 0.045),
     main = 'Distance from Missed TOI to Nearest
Delineated Region (Error Distance)',
     ylab = '', yaxt = 'n', xaxt = 'n', xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2m[,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 16)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.0004+(which(fg.prior==f)-1)*0.015,
         add = TRUE)
  }
}
axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
axis(2, at = 1.5*0.0004+(0:2)*0.015, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 7200), ylim = c(0, 0.00825),
     main = 'Histograms of Error Distances
Showing the Full Range',
     ylab = '', yaxt = 'n', xaxt = 'n',
     xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2m[,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 80)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.0001+(which(fg.prior==f)-1)*0.0027,
         add = TRUE)
  }
}
axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
axis(2, at = 1.5*0.0001+(0:2)*0.0027, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
@
%<<exp2mlegendnotrue,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Distributions of error distance for the medium site.}
%\label{exp2mres3}
%\end{figure}

%\begin{figure}[p!]
<<exp2mresults6,eval=FALSE,cache=TRUE,dependson='exp2mresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

plot(NULL, xlim = c(0, 2500), ylim = c(0, 0.059),
     main = 'Error Distances for Realization 1,209',
     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2m['r1209' ,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 16)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00075+(which(fg.prior==f)-1)*0.019,
         add = TRUE)
  }
}
axis(2, at = 1.5*0.00075+(0:2)*0.019, tick = FALSE,
...
plot(NULL, xlim = c(0, 2500), ylim = c(0, 0.036),
     main = 'Error Distances for Realization 1,405',
     ylab = '', yaxt = 'n', xaxt = 'n',
     xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2m['r1405', paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 16)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00036+(which(fg.prior==f)-1)*0.012,
         add = TRUE)
  }
}
axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
axis(2, at = 1.5*0.00036+(0:2)*0.012, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 2500), ylim = c(0, 0.048),
     main = 'Error Distances for Realization 1,797',
     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2m['r1797' ,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 16)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.0005+(which(fg.prior==f)-1)*0.016,
         add = TRUE)
  }
}
axis(2, at = 1.5*0.0005+(0:2)*0.016, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))

plot(NULL, xlim = c(0, 2500), ylim = c(0, 0.042),
     main = 'Error Distances for Realization 2,924',
     ylab = '', yaxt = 'n', xaxt = 'n',
     xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2m['r2924', paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 16)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00036+(which(fg.prior==f)-1)*0.014,
         add = TRUE)
  }
}
axis(1, at = axTicks(1), labels = prettyNum(axTicks(1), big.mark = ','))
axis(2, at = 1.5*0.00036+(0:2)*0.0014, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
@
%<<exp2mlegendnotrue,fig.height=0.2,out.height='0.2in'>>=
%@
...
%\begin{figure}[t!]
<<exp2mresults7,eval=FALSE,cache=TRUE,dependson='exp2mresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')
...
abline(h = area(union.owin(TAs.medh, TA2.medh))/43560, col = 'darkred')
@
%<<exp2mlegendcirc,fig.height=0.4,out.height='0.4in'>>=
%@
%\caption{Plots illustrating the sampling effort and efficiency for the
%medium site.}
%\label{exp2mres5}
...
%\subsection{Hard Site Results}
%\label{results2h}

%As in the plot of the number of regions, the median and mean area plots show
%the sample plans forming groups for the two larger and two smaller target
%area sizes~(Figure~\ref{exp2hres1}, bottom). Plans based on T1 or the
%underestimated size generally produce regions of the right side.

%Error distances tend to be close to 0 feet, but large error distances
%arise when sampling plans using the two largest target area sizes fail to
%detect the small target areas~(Figures~\ref{exp2hres3} and \ref{exp2hres4}).
%discuss error dist for small for rep 1706
%The false positive proportions are roughly the same for all
%analyses~(Figure~\ref{exp2hres5}, left). The false positive proportions are
%slightly higher for the hard site than for the medium site because the
%delineated regions tend to include some of the clustered background noise.
%Sampling plans with more transects have less variability in the amount
%of area that is delineated~(Figure~\ref{exp2hres5}, right). However,
%additional transect length still results in more area being delineated
%than necessary.

%\begin{figure}[b!]
<<exp2hresults2,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$detect[results2h$Target==t & results2h$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.9+(which(fg.prior==f)-1)*45,
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$detectarea[results2h$Target==t & results2h$fg==f]/area(union.owin(TAs.medh, TA2.medh)),
              plot = FALSE, breaks = seq(0, 1.02, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (4-which(targets==t))+(which(fg.prior==f)-1)*40,
...
axis(2, at = 1.5+(0:2)*40, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
@
%<<exp2hlegendnotrue,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Overall TOI detection rates and numbers of distinct regions
%delineated for the hard site.}
%\label{exp2hdetect}
%\end{figure}

%\begin{figure}[p!]
<<exp2hresults2b,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$detect3[results2h$Target==t & results2h$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)+(which(fg.prior==f)-1)*50,
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$detect5[results2h$Target==t & results2h$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (4-which(targets==t))*1.2+(which(fg.prior==f)-1)*60,
...
axis(2, at = 1.5*1.2+(0:2)*60, tick = FALSE,
     labels = paste(fg.prior, 'per Acre'))
@
<<exp2hresults2c,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5,out.width='3.25in',fig.width=3.25>>=
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$detect4[results2h$Target==t & results2h$fg==f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (4-which(targets==t))*1.2+(which(fg.prior==f)-1)*60,
...
%<<exp2hlegendnotrue,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Detection rates by target area for the hard site.}
%\label{exp2hdetectreg}
%\end{figure}

%\begin{figure}[p!]
<<exp2hresults2d,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$detectarea1[results2h$Target==t & results2h$fg==f]/area(road),
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.2+(which(fg.prior==f)-1)*10,
...
plot(NULL, xlim = c(0, 1), ylim = c(0, 25), yaxt = 'n',
     main = 'Proportion of the Ranch Area Delineated',
     ylab = 'Prior Anomaly Density Above Background', xlab = 'Proportion')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$detectarea2[results2h$Target==t & results2h$fg==f]/area(ranch),
              plot = FALSE, breaks = seq(0, 1.02, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.2+(which(fg.prior==f)-1)*10,
...
%<<exp2hlegendnotrue,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Proportions of the nuisance regions detected for the hard site.}
%\label{exp2hdetectfalse}
%\end{figure}

%\begin{figure}[p!]
<<exp2hresults3,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

stripchart(identcount ~ Target + fg, data = results2h, col = cols,
           method = 'stack', pch = '.', offset = 0.03, ylim = c(1, 13),
           main = 'Number of Distinct Regions Identified',
           xlab = 'Number of Regions',
           ylab = 'Prior Anomaly Density above Background', yaxt = 'n')
...
plot(NULL, xlim = c(0, 720), ylim = c(0, 0.06),
     main = 'Total Area of Delineated Regions',
     ylab = 'Prior Anomaly Density Above Background',
     yaxt = 'n', xlab = 'Area in Acres')
for(f in fg.prior){
  for(t in targets){
    plot(hist(results2h$identarea[results2h$Target==t & results2h$fg == f]/43560,
              plot = FALSE, breaks = seq(0, 720, 8)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.001+(which(fg.prior==f)-1)*0.02,
         add = TRUE)
...
medareas <- sapply(fg.prior, function(f){
  return(lapply(targets, function(t){
      return(sapply(areas2h[,paste0('Target', t), paste0('fg', f)],
                    function(x){
                      return(median(ifelse(is.null(x), NA, x), na.rm = TRUE))
                    })/43560)
    }))
...
abline(v = area(union.owin(TAs.medh, TA2.medh))/(43560*3), col = 'darkred')
@
%<<exp2hlegend,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Summaries of the hard site results. The red vertical lines
%represent the true values (3 TAs, total area 62.9 acres, median area
%11.5 acres, mean area 21.0 acres).}
%\label{exp2hres1}
%\end{figure}
...
%\begin{figure}[p!]
<<exp2hresults5,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2h[,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7360, 16)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.0004+(which(fg.prior==f)-1)*0.015,
         add = TRUE)
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2h[,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7360, 80)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.0001+(which(fg.prior==f)-1)*0.0027,
         add = TRUE)
...
%<<exp2hlegendnotrue,fig.height=0.2,out.height='0.2in'>>=
%@
%\caption{Distributions of error distance for the hard site.}
%\label{exp2hres3}
%\end{figure}

%\begin{figure}[p!]
<<exp2hresults6,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')

plot(NULL, xlim = c(0, 3000), ylim = c(0, 0.059),
     main = 'Error Distances for Realization 491',
     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2h['r491' ,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7360, 32)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00075+(which(fg.prior==f)-1)*0.019,
         add = TRUE)
...
plot(NULL, xlim = c(0, 3000), ylim = c(0, 0.036),
     main = 'Error Distances for Realization 1,673',
     ylab = '', yaxt = 'n', xaxt = 'n',
     xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2h['r1673', paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 32)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00036+(which(fg.prior==f)-1)*0.012,
         add = TRUE)
...
plot(NULL, xlim = c(0, 3000), ylim = c(0, 0.048),
     main = 'Error Distances for Realization 1,706',
     ylab = '', yaxt = 'n', xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2h['r1706' ,paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 32)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.0005+(which(fg.prior==f)-1)*0.016,
...
plot(NULL, xlim = c(0, 3000), ylim = c(0, 0.042),
     main = 'Error Distances for Realization 2,517',
     ylab = '', yaxt = 'n', xaxt = 'n',
     xlab = 'Distance in Feet')
for(f in fg.prior){
  for(t in targets){
    plot(hist(unlist(ndist2h['r2517', paste0('Target', t), paste0('fg', f)]), plot = FALSE,
              breaks = seq(0, 7200, 32)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.00036+(which(fg.prior==f)-1)*0.014,
...
%\begin{figure}[t!]
<<exp2hresults7,eval=FALSE,cache=TRUE,dependson='exp2hresults1',out.height='3.5in',fig.height=3.5>>=
par(mfrow = c(1, 2), mar = c(5, 4, 2, 1), bty = 'n')
...
for(f in fg.prior){
  for(t in targets){
    plot(hist(1-results2h$detectarea[results2h$Target==t & results2h$fg == f]/results2h$identarea[results2h$Target==t & results2h$fg == f],
              plot = FALSE, breaks = seq(0, 1, 0.02)), freq = FALSE,
         border = dcols[which(targets==t)],
         col = colsa6[which(targets==t)],
         voffset = (which(targets==t)-1)*0.7+(which(fg.prior==f)-1)*21,
...
abline(h = area(union.owin(TAs.medh, TA2.medh))/43560, col = 'darkred')
@
%<<exp2hlegendcirc,fig.height=0.4,out.height='0.4in'>>=
%@
%\caption{Plots illustrating the sampling effort and efficiency for the
%hard site.}
%\label{exp2hres5}
...
%\emph{If time permits, I will focus on a few representative realizations.
%Can demonstrate: residual plots, assessing Normality, choosing a window
%size by cross-validation, etc.}


\section{Issues and Considerations Related to the VSP Analysis}
\label{disc}

%\emph{TODO: Discuss how VSP fits into the UXO remediation procedure, and
%compare/contrast with other approaches (eg Neptune's Bayesian risk
%management paradigm, multiscale models, \(K\)-function or other
%distance-based cluster-detection methods)}
...
% TODO: paragraph about future use of decision analysis?

%VSP's detection probability simulation counts the target area as detected
%if any transect crosses it. This ignores the uncertainty of delineating
%the target area accurately, so a transect spacing that can detect the
%presence of a TA with high probability may not be good for mapping the
%extent of the high density region. If precise delineation is the goal of
%sampling, the window size should drive the sampling plan. A desired
%resolution should be set first, and then the transects placed to achieve
%that resolution.

%\begin{itemize}
%\item model assumptions -- do we really believe the mean is constant if the
%TAs are generated by a process separate from the background noise? etc
%\item future work: one TA -- change actual size, fg and bg density
%\end{itemize}

%It is not clear how or if the window diameter used by the flagging and
%geostatistical mapping features should relate to the window length used
%to generate detection probabilities. These windows are used for different
%purposes -- the rectangular windows are used only to see if a transect
%intersects a target area, while the circular windows are used to map
%the target areas -- so the same guidelines may not apply.

%Why use 1/6th on the window diameter as the grid size?

%Include predictors in linear model?


\section{Conclusions}
\label{conc}

%use smallest TA that would be of interest, little gain from sampling
%more than necessary, fg density not very important, have as much prior
%info as possible to sift through extra delineated area, complex background
...
%Future simulation studies should use the conceptual models of real sites
%instead of entirely hypothetical sites. This would allow the simulated
%realizations to be compared to an outcome of the real-world processes that
%generates anomalies, which could provide insight about how to best model
%the distribution of targts of interest at UXO sites.
